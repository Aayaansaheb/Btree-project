...
sellers createSeller(const char *sellerID, float rateBelow300, float rateAbove300, double totalRevenue) {
    ...
    s.transactionsubtree = createBTreeNode(true); 
    return s;
}
...
buyers createBuyer(const char *buyerID, float energyBought) {
    ...
    b.transactionsubtree= createBTreeNode(true);
    return b;
}
...
void traverseBuyers(struct buyer_btree_node *root) {
    if (root == NULL) return;
    int i;
    for (i = 0; i < root->n; i++) {
        if (!root->leaf)
            traverseBuyers(root->C[i]);
        printf("Buyer ID: %s, Energy Bought: %.2lf kWh\n", root->keys[i].buyer_id, root->keys[i].energy_bought);
    }
    if (!root->leaf)
        traverseBuyers(root->C[i]);
}

void traverseSellers(struct seller_btree_node *root) {
    if (root == NULL) return;
    for (int i = 0; i < root->n; i++) {
        printf("Seller ID: %s, Total Revenue: $%.2lf\n", root->keys[i].seller_id, root->keys[i].total_revenue);
    }
    for (int i = 0; i <= root->n; i++) {
        traverseSellers(root->C[i]);
    }
}

void traverseBuyersSellers(struct BuyerSeller_btree_node *root) {
    if (root == NULL) return;
    for (int i = 0; i < root->n; i++) {
        printf("Buyer ID: %s, Seller ID: %s, No. of Transactions: %d, Total Energy: %.2lf kWh\n",
               root->keys[i].buyerID, root->keys[i].sellerID, root->keys[i].no_of_tr, root->keys[i].total_energy);
    }
    for (int i = 0; i <= root->n; i++) {
        traverseBuyersSellers(root->C[i]);
    }
}
...
void loadSellersFromFile(const char *filename, struct seller_btree_node **sellerRoot) {
    ...
    while (fscanf(file, "%s %f %f", sellerID, &rateBelow300, &rateAbove300) == 3) {
...
int main() {
    struct BTreeNode *transactionRoot = createBTreeNode(true);
    struct seller_btree_node *sellerRoot = createSellerTreeNode(true);

    // Load transactions from file
    const char *transactionFile = "transactions.txt";
    loadTransactionsFromFile(transactionFile, &transactionRoot);

    // Load sellers from file
    const char *sellerFile = "sellers.txt";
    loadSellersFromFile(sellerFile, &sellerRoot);

    // Create buyer and seller trees from transactions
    struct buyer_btree_node *buyerRoot = createBuyerTreeNode(true);
    buildBuyerTreeFromTransactions(transactionRoot, &buyerRoot);
    struct BuyerSeller_btree_node *buyerSellerRoot = createBuyerSellerTreeNode(true);
    buildBuyerSellerTreeFromTransactions(transactionRoot, &buyerSellerRoot);
    // Create transaction subtrees for all buyers
    createTransactionSubtreesForAllBuyers(transactionRoot, buyerRoot);
    // Create transaction subtrees for all sellers
    createTransactionSubtreesForAllSellers(transactionRoot, sellerRoot);
    int choice;
    do{
        ...
        case 1:
            {
                ...
                scanf("%d %d %d %d %d %d", &timestamp.day, &timestamp.month, &timestamp.year, &timestamp.hour, &timestamp.minute, &timestamp.second);
                Transaction* t = searchTransaction(transactionRoot, transactionID);
                if (t != NULL) {
                    printf("Transaction ID already exists!\n");
                }
                else{
                    struct Transaction transaction = createTransaction(transactionID, buyerID, sellerID, energyAmount, pricePerkWh, timestamp);
                    insertTransaction(&transactionRoot,transaction);
                }
                break;
            }
        case 2:
            traverseTransactions(transactionRoot);
            break;
        case 3:
        {   
            char sellerID[20];
            printf("Enter Seller ID: ");
            scanf("%s", sellerID);
            struct sellers* seller=searchSeller(sellerRoot, sellerID);
            if (seller != NULL) {
                traverseTransactions(seller->transactionsubtree);
            } else {
        ...
        case 4:
        {
                ...
                printf("Enter Buyer ID: ");
                scanf("%s", buyerID);
                struct buyers* buyer=searchBuyer(buyerRoot, buyerID);
                if (buyer != NULL) {
                    traverseTransactions(buyer->transactionsubtree);
                } else {
        ...
        case 5:
        {
            ...
            traverseTransactionsInRange(transactionRoot, start, end);
            break;
        }
        case 6:
        {
            ...
            struct sellers* seller=searchSeller(sellerRoot, sellerID);
            if(seller!=NULL){
                totalRevenue=calculate_total_revenue(transactionRoot, sellerID);
                printf("Total Revenue for Seller %s: %.2lf\n", sellerID, totalRevenue);
            }
            else{
                printf("Seller not found!\n");
            } 
        ...
        case 7:
        {
            ...
            scanf("%lf", &maxEnergy);
            rangeTraversal_transactions(transactionRoot, minEnergy, maxEnergy);
            break;
        }
        case 8:
            traverseBuyers(buyerRoot);
            break;
        case 9:
            traverseBuyersSellers(buyerSellerRoot);
            break;
        case 0:
        {   printf("Exiting...\n");
            FILE *fp = fopen(transactionFile, "w");
            if (fp == NULL) {
                perror("Error opening file for writing");
                return 1;
            }
            saveTransactionsToFile(fp, transactionRoot);
            fclose(fp);
            break;
        }
        default:
